...
ecs = [ecosys_config(4, N0=N, seed=42) for N in N0s];
...
for (i, ec) in enumerate(ecs)
    ...
    for (j, σ) in enumerate(σs)
        ...
        vols[j] = volume_range_EFD(p, :matr, Q_ranges_t[i], n_sample=2*10^4)  # this is a Vector
    end
    vols_total[i] = vols
end
...
for (i, ec) in enumerate(ecs)
    ...
    for (j, σ) in enumerate(σs)
        p = generate_model_system(ec, σ)
        vols[j] = volume_range_EFD(p, :init, Q_ranges_i[i])  # this is a Vector
    end
    vols_indiv[i] = vols
end
