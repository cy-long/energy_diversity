...
# df <- read_csv("analysis/trophic_linear_m1d1N1.csv")
df <- read_csv("analysis/trophic_linear_am.csv")
df$type <- df$tr_type %>% gsub("(^|[[:space:]])([[:alpha:]])", "\\1\\U\\2", ., perl=TRUE) %>% as.factor()
df$norm_vol[is.nan(df$norm_vol)] <- 0
df <- df %>% rename(S_ind = S) # temporary
...
p <- ggplot() +
  geom_line(data = df,
    aes(x = S_ind, y = vol_d / S_ind^3, group = interaction(type, rep_id), color = type),
  ...
  coord_cartesian(xlim = c(0,20.0), ylim = c(0,4.0)) +
  labs(x = "Individual Power Cap", y = "Norm. Vol.", title = "allometric m") +
  facet_wrap(~ type) +
