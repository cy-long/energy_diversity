...
for i in eachindex(σs)
    ...
    vols = volume_range_EFD(p_i, P_range, n_sample=10^4)
    devols = [S^p_i.K / (factorial(p_i.K) * prod(p_i.N⁰)) for S in P_range];
    push!(vols_all, vols)
...
plot(P_range, prob_pers, xlabel="Total Energy Supply", ylabel="P[Persistence]", legend=false, color = :olivedrab, ylim=(0,0.5), linewidth=1.5, framestyle=:box, foreground_color_border=:black, borderlinewidth=1, grid=false)
